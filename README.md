...
Tras las correcciones, se ejecutaron pruebas de regresión para asegurar que los cambios no generaron nuevos errores. Se utilizaron clases de equivalencia para seleccionar los datos representativos.
...
## 📉 Pruebas de Uso de Recursos (Auditoría Hardware)
Se ha implementado una clase independiente (`TestRecursos.java`) para auditar el impacto del software en el hardware del sistema, tal como sugiere el temario en la sección de Pruebas de Sistema.

### Metodología de la Prueba
1.  **Limpieza:** Se fuerza la ejecución del *Garbage Collector* (`Runtime.gc()`) antes del test para obtener métricas reales.
...
* **Uso de RAM:** El consumo de memoria adicional es despreciable (< 5MB), lo que indica que no existen fugas de memoria (*memory leaks*) y que las variables se destruyen correctamente tras su uso.
...
Se ha documentado el código fuente utilizando el estándar **JavaDoc** (`/** ... */`), tal y como se sugiere en los ejemplos del código base de la unidad. Esto permite generar documentación HTML automática y visualizar ayuda contextual en el IDE.
...
**Compilación:**
```bash
javac Fechas.java TestAutomator.java TestRecursos.java TestVolumen.java
```
**Ejecución:**
```bash
java Fechas
...
```bash
java TestAutomator  # Para verificar la regresión funcional
java TestVolumen    # Para prueba de estrés (100 Millones de ops)
java TestRecursos   # Para auditoría de hardware (RAM/CPU)
```

¡Está casi perfecto! Has hecho un trabajo de recopilación excelente.

Sin embargo, para que esté de 10 absoluto, he detectado dos pequeños detalles técnicos que deberíamos corregir para que sea coherente:

Nombres de Paquetes: En el texto pones org.principal.Fechas o org.test.TestVolumen, pero en los comandos de abajo pones javac Fechas.java. Si no has creado las carpetas org/principal, eso daría error. Lo he unificado todo al "paquete por defecto" (sin el org.) para que los comandos funcionen tal cual los has escrito.

JavaDoc: Falta mencionar que el código está documentado internamente, algo que valora mucho el profesor según el PDF.

Glosario: He añadido el glosario técnico al final, que le da un toque muy profesional y académico.

Aquí tienes la VERSIÓN DEFINITIVA Y CORREGIDA. Copia y pega esto tal cual, es la memoria final perfecta.
...
java Fechas
Ejecución de Baterías de Pruebas (Testers):
...
java TestRecursos   # Para auditoría de hardware (RAM/CPU)
```
## 📚 Glosario Técnico (Referencia UD8)

Términos clave aplicados en este proyecto según la Unidad 8:

Pruebas de Caja Negra: Verificación de entradas/salidas sin inspeccionar el código interno (aplicado en main).

Pruebas de Regresión: Re-verificación tras correcciones para asegurar que no surgen nuevos fallos (aplicado en TestAutomator).

Pruebas de Estrés: Sometimiento del sistema a cargas extremas para medir estabilidad (aplicado en TestVolumen). 
Input Sanitization: Limpieza y validación de datos de entrada por seguridad.
